...
def check_amp(model):
    ...
    try:
        ...
        amp_model = Path("yolo26n.pt")
        if not amp_model.exists():
            LOGGER.warning(f"{prefix}checks skipped. Local yolo26n.pt not found; skipping download. {warning_msg}")
            return True

        assert amp_allclose(YOLO(str(amp_model)), im)
